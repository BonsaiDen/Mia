        ...
        ast.getAssignmentToNode(node).forEach(function(assign) {
            ...
            if (assign.left.type === 'MemberExpression') {
                ...
                // Static members
                    ...
                    // TODO detect factories and other things
...
function getFunctionType(func) {
    ...
    if ((func.properties.length || func.superConstrutorCalls.length) && func.isUpper) {
        return 'Class';
...
function tag(name, comment, node) {
    ...
    if (ast.isFunction(node)) {
        ...
        // Plain functions
        if (func.type === 'Function') {
            return new tags.FunctionTag(node.$uid, name, comment, func.params);

        // Factories
        } else if (func.type === 'Factory') {
            return new tags.FactoryTag(node.$uid, name, comment, func.params);
        }
