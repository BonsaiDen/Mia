...
var Parser = {
    ...
    parseTags: function(tags, names, path, node, config) {
        ...
        if (tags.hasOwnProperty(uid)) {
            //console.log('aliased name for tag:', uid, tags[uid].name, 'to', path);
            names[uid].push(path);
    ...
    addTag: function(tags, names, tag, config) {
        ...
        if (typeof tag.comment === 'string') {
            tag.annotate(function(tag) {
                var annotation = Parser.parseAnnotations(tag.comment, config)[0];
                if (annotation) {
    ...
    parseAnnotations: function(node, config) {
        return Parser.collect('annotation', config.annotation, [node]);
    },
