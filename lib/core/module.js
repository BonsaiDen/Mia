...
var mod = {
    ...
    parseTags: function(tags, names, path, node, config) {
        ...
        if (tags.hasOwnProperty(uid)) {
            ...
            var result = mod.applyParsers('source', config.source, [path, node]);
            if (result.length) {
    ...
    parseExports: function(moduleName, ast, config) {
        return mod.applyParsers('module', config.module, [moduleName, ast.body]);
    },

    parseAnnotations: function(node, config) {
        return mod.applyParsers('annotation', config.annotation, [node]);
    },
    ...
    executeParser: function(type, parser, data) {
        ...
        try {
            ...
            if (typeof parser === 'string') {
                if (mia.parsers[type].hasOwnProperty(parser)) {
                    return mia.parsers[type][parser].parse.apply(null, data);
